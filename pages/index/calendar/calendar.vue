...
.calendar-info>view.month .month-inner{
    position: absolute;
    left:0;
    top:0;
    height:240px;
...
.calendar-info>view.month .month-inner>span{
    display: block;
    font-size: 20px;
    height:35px;
    width:100px;
...
<template>
    <view class="calendar">
		<!-- 详情界面展开样式 -->
		<view v-if="tempIfSelectedSame" style="position:relative; height: 10vh; width: 100%;">
			<uni-icons type="left" size="60px" color="#919ec8" @click="backToCalendar"></uni-icons>
			<view style="position: absolute; top:8px; left: 50%; transform: translateX(-50%); ;font-size: 32px; color: #919ec8;">{{this.year}}. {{this.month+1}}</view>
		</view>
		
		<!-- 日历界面 -->
        <view class="calendar-tools" v-if="!tempIfSelectedSame">
            <span class="calendar-prev" @click="prev">
				<uni-icons type="left" size="60px" color="#919ec8"></uni-icons>
        ...
        <tr v-for="(day,k1) in days" style="{'animation-delay',(k1*30)+'ms'}">
            <td v-for="(child,k2) in day" :class="{'selected':child.selected,'disabled':child.disabled}" @click="select(k1,k2,$event)">
				...
				<view v-if="child.todoEvent" class="todoD">
					<text class="todoDetail" v-for="(y,index) in child.todoEvent" v-if="index<3" :style="{'text-decoration': y[1] ? 'line-through': ''}">{{y[0]}}</text>
				</view>
				<!-- <view class="todoTask" v-if="child.day== v-for="(item,index) in todoData"">{{}}</view> -->
             <!--   <view class="text" v-if="child.eventName!=undefined">{{child.eventName}}</view>
                <view class="text" :class="{'isLunarFestival':child.isLunarFestival,'isGregorianFestival':child.isGregorianFestival}" v-if="lunar">{{child.lunar}}</view> -->
            </td>
        </tr>
        </tbody>
		
		<tr v-if="tempIfSelectedSame">
		    <td v-for="(child,k2) in keepLine" :class="{'selected':child.selected,'disabled':child.disabled}" style="font-size:20px" @click="select1(k2,$event)">
		        <span :class="{'red':k2==0||k2==6||((child.isLunarFestival||child.isGregorianFestival) && lunar)}">{{child.day}}</span>
				<!-- <view v-if="child.todoEvent" class="todoD">
					<text class="todoDetail" v-for="(y,index) in child.todoEvent" v-if="index<3" :style="{'text-decoration': y[1] ? 'line-through': ''}">{{y[0]}}</text>
				</view> -->
		...
		<view v-if="tempIfSelectedSame">
		<view style="position: absolute; top: 21%; width: 100%; height: 100%; background-color: aliceblue; display: flex; text-align: center;
		flex-direction: column;" >
		<view style="display: flex; width: 100%; flex-wrap: wrap;  background-color: white; padding: 0px 5px 0px; border-top: solid blue 1px; margin-top:-1px;" >
			<long-press style="padding: 4px 0px 8px;" v-for="everyThing in dataWithoutTime" :key="everyThing.id" :et="everyThing" >
			</long-press>
		</view>
			<view style="display: flex; height: 100%; flex-direction: row;">
				<!-- 展开界面左边时间栏 -->
				<view style="display: flex; flex: 2; background-color: antiquewhite;">
					<view v-for="everyThing in currentEvent.selectedMin" :key="everyThing.id">{{everyThing.title}}</view>
				</view>
				
				<view style="display: flex; flex:5;">
					<long-press v-for="everyThing in dataWithTime" :key="everyThing.id" :et="everyThing" >
					</long-press>
				</view>
			</view>
		</view>
        ...
        <view class="calendar-years" :class="{'show':yearsShow}">
...
export default {
    props: {
        ...
        weeks: {
            ...
            default:function(){
                return 
				// window.navigator.language.toLowerCase() == "zh-cn"?['日', '一', '二', '三', '四', '五', '六']:['Sun', 'Mon', 'Tue', 'Wed', 'Thu', 'Fri', 'Sat']
            }
        },
        // 自定义月份
        months:{
            ...
            default:function(){
                return
				 // window.navigator.language.toLowerCase() == "zh-cn"?['一月', '二月', '三月', '四月', '五月', '六月', '七月', '八月', '九月', '十月', '十一月', '十二月']:['January', 'February', 'March', 'April', 'May', 'June', 'July', 'August', 'September', 'October', 'November', 'December']
            }
        },
        // 自定义事件
        events:  {
	...
	computed:{
		dataWithTime:function(){
			return this.currentEvent.filter((x)=>{
				return x.selectedMin!=''
			})
		},
		dataWithoutTime:function(){
			return this.currentEvent.filter((x)=>{
				return x.selectedMin=='' & x.selectedHour==''
				console.log("dataWithoutTime is recomputed", dataWithoutTime)
			})
		},
	},
    watch:{
    ...
    mounted() {
		uni.getStorage({
			key:'todoBox1',
			success:(e)=>{
				//将所有data数据加入todoData数据集
				this.todoData=this.todoData.concat(e.data);
	...
	watch:{
		todoData:{
			deep:true,
			handler(value){
				console.log('the value in calendar changed', value)
				value.forEach((temp)=>{
					// console.log("temp",temp)
					if(!temp.importanceLevel & temp.emergenceLevel){
						this.tempBox1=this.tempBox1.concat(temp)
						// console.log(this.tempBox1,"tempBox1")
					}
					if(temp.importanceLevel & temp.emergenceLevel){
						this.tempBox2=this.tempBox2.concat(temp)
						// console.log(this.tempBox2,"tempBox2")
					}
					if(!temp.importanceLevel & !temp.emergenceLevel){
						this.tempBox3=this.tempBox3.concat(temp)
						// console.log(this.tempBox3,"tempBox3")
					}
					if(temp.importanceLevel & !temp.emergenceLevel){
						this.tempBox4= this.tempBox4.concat(temp)
						// console.log(this.tempBox4,"tempBox4")
					}
				}),
				uni.setStorage({
					key: 'todoBox1',
		...
		currentEvent:{
			deep:true,
			handler(value){
				console.log('the currentEvent in calendar changed', value)
		}
		},
	},
    ...
    methods: {
            ...
            for (i = 1; i <= lastDateOfMonth; i++) {
                ...
                if (this.range) { // 范围
					...
					this.todoData.forEach(x=>{
						// console.log(x)
						if(x.selectedDate==i && x.selectedMonth==this.month+1 && x.selectedYear==this.year){
							todoE.push([x.title,x.todoFinished]);
							todoEAll.push(x)
							// console.log(x,"事件！", x.selectedDate)
							// todoEFinished.push();
							 // console.log(i,todoE,"todoE")
							 }
                    ...
                    if (parseInt(seletSplit[0]) == this.year && parseInt(seletSplit[1]) - 1 == this.month && parseInt(seletSplit[2]) == i) {
                        ...
                        temp[line].push(Object.assign(
                            {day: i,selected: true,todoEvent:todoE, todoEAlle:todoEAll},
                            this.getLunarInfo(this.year,this.month+1,i),
                            this.getEvents(this.year,this.month+1,i),
                        ))
                     ...
                     // 没有默认值的时候显示选中今天日期
						...
						// console.log("普通日期")
                        let options = Object.assign(
                            {day: i, selected:false, todoEvent:todoE, todoEAlle:todoEAll},
                            this.getLunarInfo(this.year,this.month+1,i),
                            this.getEvents(this.year,this.month+1,i),
                        )
						todoE=[]
        ...
        select(k1, k2, e) {
            ...
            if (this.range) {
                ...
                // 取消上次选中
				let tempData=this.today;
				// console.log(tempData,"this.today1");
				...
				// console.log(this.today,"this.today2");
                // 设置当前选中天
                this.days[k1][k2].selected = true
                this.day = this.days[k1][k2].day
				// console.log(this.day,this.days,k1,this.days[k1])
                this.today = [k1, k2]
				// console.log(this.today,"this.today3");
				// console.log(this.today,tempData);
				// console.log(this.temp);
				//判断是否与上次选中日期相同，如相同，展开
				if(this.today[0] == tempData[0] & this.today[1] == tempData[1] ){
					this.keepLine=this.days[k1]
					this.tempIfSelectedSame=true
					this.currentEvent=this.days[k1][k2].todoEAlle
					// this.currentEvent=this.keepLine[k2].todoEAlle
				}
				else{
					this.tempIfSelectedSame=false
				}
                this.$emit('select',[this.year,this.zero?this.zeroPad(this.month + 1):this.month + 1,this.zero?this.zeroPad(this.days[k1][k2].day):this.days[k1][k2].day],this.tempIfSelectedSame)
            }
        },
		select1(k2, e) {
		    // console.log(this.keepLine[k2])
		        // 取消上次选中
				let tempData=this.today;
				// console.log(tempData,"this.today1");
		        if (this.today.length > 0) {
				...
				// console.log(this.today,"this.today2");
		        // 设置当前选中天
		        this.keepLine[k2].selected = true
				// console.log(this.keepLine[k2].todoEAlle)
				this.currentEvent=this.keepLine[k2].todoEAlle
